...
<body class="bg-gray-100 p-8">
    ...
    <h1 class="text-2xl font-bold mb-4">Edit Condition</h1>

    <form method="POST" action="{{ route('conditions.update', $condition->id) }}" class="bg-white p-6 rounded-lg shadow-md mb-4">
        @csrf
        @method('PUT')
        <div class="mb-4">
            <label for="name" class="block text-gray-700">Name:</label>
            <input type="text" name="name" id="name" class="w-full mt-2 p-2 border rounded-lg" value="{{ $condition->name }}" required>
        </div>
